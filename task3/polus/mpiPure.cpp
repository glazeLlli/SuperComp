	...
	for (int i = 1; i < M; i++) {
		for (int j = 1; j < N; j++) {
			...
			//at least part of it is inside D
				...
				if (nodeType[i * N + j] == 1 && nodeType[(i - 1) * M + j] == 0) {
					Fij[i * N + j] = 1 * yU * xR / (h_1 * h_2);
				}
				else if (nodeType[i * N + j] == 1 && nodeType[(i - 1) * M + j] == 1) {
					Fij[i * N + j] = 1 * xR / h_1;
				}
				else if (nodeType[i * N + j] == 0 && nodeType[(i - 1) * M + j] == 1) {
					if (yU <= 4) {
						Fij[i * N + j] = 1 * (h_1 * h_2 - (-xL) * h_2 - (xR - (-3. / 4 * yU + 3)) * (yU - (-4. / 3 * xR + 4)) / 2) / (h_1 * h_2);
					}
					else {
						Fij[i * N + j] = 1 * (((-4. / 3 * xR + 4) - yD + 4 - yD) * xR / 2) / (h_1 * h_2);
					...
					if (yU < 4) {
						if (4 * xR + 3 * yD - 12 > 0) {
							if (yD >= 0) {
								Fij[i * N + j] = 1 * (((-3. / 4 * yU + 3) + (-3. / 4 * yD + 3)) * h_2 / 2) / (h_1 * h_2);
							}
							else {
								Fij[i * N + j] = 1 * (((-3. / 4 * yU + 3) + 3) * yU / 2) / (h_1 * h_2);
							}
						}
						else if (xR < 3) {
							Fij[i * N + j] = 1 * (xR * yU - (xR - (-3. / 4 * yU + 3)) * (yU - (-4. / 3 * xR + 4)) / 2) / (h_1 * h_2);
						}
						else {
							Fij[i * N + j] = 1 * (((-3. / 4 * yU + 3) + 3) * yU / 2) / (h_1 * h_2);
						}
						...
						if (yD > 0) {
							Fij[i * N + j] = 1 * ((-3. / 4 * yD + 3) * (4 - yD) / 2) / (h_1 * h_2);
						}
						else {
							Fij[i * N + j] = 6 / (h_1 * h_2);
						}
				...
				if (yD < 0) {
					if (nodeType[i * N + j - 1] == 1 && nodeType[i * N + j] == 1) {
						...
						if (xR <= 3) {
							Fij[i * N + j] = 1 * (yU * h_1 - (xR - (-3. / 4 * yU + 3)) * (yU - (-4. / 3 * xR + 4)) / 2) / (h_1 * h_2);
						}
						else {
						...
						if (xL > 0) {
							Fij[i * N + j] = 1 * ((-4. / 3 * xL + 4) * (3 - xL) / 2) / (h_1 * h_2);
						}
						else {
							Fij[i * N + j] = 6 / (h_1 * h_2);
						}
					...
					if (nodeType[i * N + j - 1] == 0 && nodeType[(i - 1) * M + j] == 0 && nodeType[i * N + j] == 0) {
						if (h_1 * h_2 >= 12) {
							Fij[i * N + j] = 6 / (h_1 * h_2);
						}
						else {
							Fij[i * N + j] = 1 * (((-3. / 4 * yD + 3) - xL) * ((-4. / 3 * xL + 4) - yD) / 2) / (h_1 * h_2);
						}
					}
					else if (nodeType[i * N + j - 1] == 1 && nodeType[(i - 1) * M + j] == 0 && nodeType[i * N + j] == 0) {
						Fij[i * N + j] = 1 * ((((-3. / 4 * yU + 3) - xL) + ((-3. / 4 * yD + 3) - xL)) * (h_2) / 2) / (h_1 * h_2);

					}
					else if (nodeType[i * N + j - 1] == 0 && nodeType[(i - 1) * M + j] == 1 && nodeType[i * N + j] == 0) {
						Fij[i * N + j] = 1 * ((((-4. / 3 * xR + 4) - yD) + ((-4. / 3 * xL + 4) - yD)) * (h_1) / 2) / (h_1 * h_2);
					}
					else if (nodeType[i * N + j - 1] == 1 && nodeType[(i - 1) * M + j] == 1 && nodeType[i * N + j] == 0) {
						Fij[i * N + j] = 1 * (h_1 * h_2 - (xR - (-3. / 4 * yU + 3)) * (yU - (-4. / 3 * xR + 4)) / 2) / (h_1 * h_2);
...
int main(int argc, char* argv[]) {
	...
	if (rank == 0) {
		string txtFileName("MPIparallelInfo"), csvFileName("MPIdata");
		txtFileName += to_string(M) + " " + to_string(N) + " " + to_string(numProcs) + ".txt";
		csvFileName += to_string(M) + " " + to_string(N) + " " + to_string(numProcs) + ".csv";
